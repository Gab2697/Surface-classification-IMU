function outdoor_gait_cycle_data_Knee(fld,n_cycles)
...
HSevts=fieldnames(data.JointAnglesZXY_Right_Knee_Flexion_Extension.event);
...
for i = 1:length(HSevts)
    if contains(HSevts{i}, 'RHS')
        r = data.JointAnglesZXY_Right_Knee_Flexion_Extension.event.(HSevts{i});
        HS(i)  =r(1);
    end
end
